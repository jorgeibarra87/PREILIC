...
$sql="INSERT INTO `plico`.`recursoasignado` (`id_recursoasignado`, `id_evento`, `id_recurso`, `cantidad`, `valor`, `subtotal`) VALUES (NULL, '$idevento', '$rec', '$can', '$val', '$subtotal');";

$sqltres="UPDATE  `eventos` SET  `costo` =  '$costo' WHERE `eventos`.`id_evento` ='$idevento'";

if($rs=$base->dosql($sql)and $rsss=$base->dosql($sqltres))
{
echo"<center>EL RECURSO SE A ASIGNADO AL EVENTO</center><BR>";
echo"<center><a href=\"../logica/asignarrecursos.php\"><IMG  src=\"../presentacion/volver.jpg\" ></a></center><BR><BR>";
}
else{
