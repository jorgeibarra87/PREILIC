...
$querys=mysql_query("SELECT `usuarios`.`login`,`usuarios`.`id_usuario` FROM `usuarios`where `usuarios`.`login`='$usuario'",$conexion);
$filass=mysql_num_rows($querys);
if($filass==1){
$rows = @mysql_fetch_array($querys);
$iduser = $rows['id_usuario'];
}

$query=mysql_query("SELECT `eventos`.`id_evento`,`eventos`.`num_radicado`,`eventos`.`beneficio`,`eventos`.`resultado`,`eventos`.`porcentaje`,`eventos`.`costo` FROM eventos where `eventos`.`num_radicado`='$radicado'",$conexion);
$filas=mysql_num_rows($query);
if($filas==1){
$row = @mysql_fetch_array($query);
$subresultado = $row['resultado'];
$idevento = $row['id_evento'];
$benefic = $row['beneficio'];
$porcent = $row['porcentaje'];
$costo = $row['costo'];
}
$resultado=$subresultado+$subtotal;

$beneficio=$resultado-$costo;
$porcentaje=($beneficio/$resultado)*100;

$base=new base('localhost','root','','plico');
$sql="INSERT INTO `plico`.`resultado` (`id_resultado`, `id_evento`, `id_recurso`, `cantidad`, `valor`, `subtotal`) VALUES (NULL, '$idevento', '$rec', '$can', '$val', '$subtotal');";

$sqldos="UPDATE  `eventos` SET  `resultado` =  '$resultado' WHERE `eventos`.`id_evento` ='$idevento'";

$sqltres="UPDATE  `eventos` SET  `porcentaje` =  '$porcentaje' WHERE `eventos`.`id_evento` ='$idevento'";

$sqlcuatro="UPDATE  `eventos` SET  `id_usuario_res` =  '$iduser' WHERE `eventos`.`id_evento` ='$idevento'";

$sqlcinco="UPDATE  `eventos` SET  `beneficio` =  '$beneficio' WHERE `eventos`.`id_evento` ='$idevento'";

if($rs=$base->dosql($sql)and $rss=$base->dosql($sqldos) and $rsss=$base->dosql($sqltres) and $rssss=$base->dosql($sqlcuatro) and $rsssss=$base->dosql($sqlcinco))
{
echo"<center>EL RESULTADO SE A ASIGNADO AL EVENTO</center><BR>";
echo"<center><a href=\"../logica/asignarresultados.php\"><IMG  src=\"../presentacion/volver.jpg\" ></a></center><BR><BR>";
}
else{
