...
<title>borrar resultado asignado</title>
...
$query=mysql_query("SELECT `resultado`.`id_evento`,`resultado`.`id_resultado`,`resultado`.`subtotal` FROM resultado where `resultado`.`id_resultado`='$ID'",$conexion);
$filas=mysql_num_rows($query);
if($filas==1){
$row = @mysql_fetch_array($query);
$evento = $row['id_evento'];
$idresasig = $row['id_resultado'];
$subtotal = $row['subtotal'];
}
$querys=mysql_query("SELECT `eventos`.`id_evento`,`eventos`.`num_radicado`,`eventos`.`beneficio`,`eventos`.`resultado`,`eventos`.`porcentaje`,`eventos`.`costo` FROM eventos where `eventos`.`id_evento`='$evento'",$conexion);
...
$sqltres="UPDATE  `eventos` SET  `resultado` =  '$resultado' WHERE `eventos`.`id_evento` ='$evento'";

$sqldos="UPDATE  `eventos` SET  `beneficio` =  '$beneficio' WHERE `eventos`.`id_evento` ='$evento'";

$sqluno="UPDATE  `eventos` SET  `porcentaje` =  '$porcentaje' WHERE `eventos`.`id_evento` ='$evento'";

$sql="DELETE FROM `resultado` WHERE `id_resultado` =$ID";



if($rsss=$base->dosql($sqltres)and $rss=$base->dosql($sqldos) and $rs=$base->dosql($sqluno) and $r=$base->dosql($sql)){


echo"<center><p class=\"Estilo12\">EL RECURSO ASIGNADO SE A BORRADO</p></center><BR>";
echo"<center><a href=\"../logica/principal.php\"><IMG  src=\"../presentacion/volver.jpg\" ></a></center>";
}
...
echo"<center><a href=\"../logica/ediasigrec.php\"><IMG  src=\"../presentacion/volver.jpg\" ></a></center>";
}	
