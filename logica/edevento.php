...
$sqlev=mysql_query("SELECT `eventos`.`num_radicado`,`eventos`.`nom_evento`,`eventos`.`fecha_inicio`,`eventos`.`fecha_fin`,`lugar`.`lugar`,`solicitante`.`solicitante` FROM eventos
LEFT JOIN `plico`.`lugar` ON `eventos`.`id_lugar` = `lugar`.`id_lugar` 
LEFT JOIN `plico`.`solicitante` ON `eventos`.`id_solicitante` = `solicitante`.`id_solicitante` where `num_radicado`=$rad",$conexion);
$f=mysql_num_rows($sqlev);
if($f==1){
$row = @mysql_fetch_array($sqlev);
$radicado = $row['num_radicado'];
$nev = $row['nom_evento'];
...
$rs=$base->dosql($sql);
$rss=$base->dosql($sqls);
		echo"<center><p class=\"Estilo12\">EDITAR EVENTO:</p></center><BR>";
   echo"<center><form id=\"form1\" name=\"form1\" method=\"post\" action=\"../logica/updateeven.php?ID=$rad\" onsubmit=\"return validar()\">
  <table width=\"420\" border=\"1\">
    <tr>
      <td width=\"155\"><center><BR>NUMERO DE RADICADO:<BR><BR></center> </td>
      <td width=\"232\"><label>
        <input type=\"text\" name=\"radicado\" value=\"$radicado\"/>
      </label></td>
    </tr>
	<tr>
      <td width=\"155\"><center><BR>NOMBRE EVENTO:<BR><BR></center> </td>
      <td width=\"232\"><label>
        <input type=\"text\" name=\"nomevento\" value=\"$nev\"/>
      </label></td>
    </tr>
    <tr>
      <td><center><BR>FECHA INICIO:<BR><BR></center> </td>
      <td><label>
        <input readonly type=\"text\"name=\"fechaini\" id=\"datepicker\" value=\"$fechaini\"/>
      </label></td>
    </tr>
	<tr>
      <td><center><BR>FECHA FIN:<BR><BR></center> </td>
      <td><label>
        <input readonly type=\"text\"name=\"fechafin\" id=\"datepickerr\" value=\"$fechafin\"/>
      </label></td>
    </tr>
    <tr>
      <td><center><BR>LUGAR:<BR><BR></center> </td>
      <td><select name=\"lugar\" value=\"$lugar\">";
    while(!$rs->EOF)
{
	    $lug=$rs->fields['lugar'];
  ...
  echo"</select></td>
    ...
    <tr>
      ...
      <td>
  <select name=\"solicitante\" value=\"$solicitante\">";
      while(!$rss->EOF)
{
