...
<title>editar el recurso asignado</title>
...
echo"<center><p\">EVENTO Y RECURSOS ASIGNADOS </p></center><BR>";
...
echo"</center><br><br>";
$sqldos="SELECT `recursoasignado`.`id_recursoasignado`,`recurso`.`nombre_recurso`,`recursoasignado`.`cantidad`,`recursoasignado`.`valor`,`recursoasignado`.`subtotal` FROM recursoasignado
LEFT JOIN `plico`.`recurso` ON `recursoasignado`.`id_recurso` = `recurso`.`id_recurso` where `recursoasignado`.`id_evento`='$idevento'";
$page = new ADODB_Pager($db,$sqldos);
echo"<center><p class=\"Estilo12\">RECURSOS ASIGNADOS: </p></center><BR>";
echo"<center>";
$page->Render($rows_per_page=10);

echo"<BR><BR><center>";
echo"<center>ELIJA EL RECURSO ASIGNADO QUE DESEA BORRAR:</center><BR><BR>";
$base=new base('localhost','root','','plico');
$rs=$base->dosql($sqldos);
 echo"<center><form id=\"form1\" name=\"form1\" method=\"post\" action=\"../logica/borrarecasig.php\">
<select name=\"recasig\">";
    while(!$rs->EOF)
{
	    $recasig=$rs->fields['nombre_recurso'];
		$irecasig=$rs->fields['id_recursoasignado'];
		echo"<option value=\"$irecasig\">$irecasig</option>";
		$rs->MoveNext();
}		
  echo"</select><BR><BR>
