...
<title>ver asignacion de resultados</title>
...
$consul=mysql_query("SELECT `permisos_perfil`.`id_perfil`,`permisos_perfil`.`id_permiso` FROM `permisos_perfil`where `permisos_perfil`.`id_perfil`='$perfil'and `permisos_perfil`.`id_permiso`='12'",$conexion);
$fil=mysql_num_rows($consul);
if($fila==1||$fil==1){
	echo"<BR><BR><center><p class=\"Estilo12\">BORRAR RESULTADOS ASIGNADOS ELEGIR: </p></center><BR>";
...
echo"<center>";$pager->Render($rows_per_pager=10);echo"</center>";
echo"<center>";
echo"<BR><BR><center>DIGITE EL NUMERO DE RADICADO DEL EVENTO PARA BORRAR RESULTADOS ASIGNADOS:</center><BR>";
echo"</p><center>
<form id=\"form1\" name=\"form1\" method=\"post\" action=\"../logica/ediasigs.php\">
  <label>NUM_RADICADO:
