...
if(document.form1.perfil.value.length==0){ 
alert("campo vacio"); 
return false; 
}
else{
...
$per=$_POST['perfil'];
$sqlp=mysql_query("SELECT `id_perfil`, `nom_perfil` FROM `perfil` where `id_perfil`=$per",$conexion);
$f=mysql_num_rows($sqlp);
if($f==1){
$row = @mysql_fetch_array($sqlp);
$id = $row['id_perfil'];
$nom = $row['nom_perfil'];
}
...
$rs=$base->dosql($sql);
		echo"<center><p class=\"Estilo12\">EDITAR PERFIL:</p></center><BR>";
   echo"<center><form id=\"form1\" name=\"form1\" method=\"post\" action=\"../logica/updateperfil.php?ID=$per\" onsubmit=\"return validar()\">
  <table width=\"420\" border=\"1\">
    <tr>
      <td width=\"155\"><center><BR>NOMBRE DEL PERFIL:<BR><BR></center> </td>
      <td width=\"232\"><label>
        <input type=\"text\" name=\"perfil\" value=\"$nom\"/>
      </label></td>
    </tr>
	<tr>
