...
if(document.form1.login.value.length==0&&document.form1.pass.value.length==0&&document.form1.funcionario.value.length==0){ 
alert("campos vacios"); 
return false; 
}
...
else if(document.form1.funcionario.value.length==0){
...
$us=$_POST['usuario'];
$sqlus=mysql_query("SELECT `usuarios`.`id_usuario`,`usuarios`.`login`,`usuarios`.`pass`,`usuarios`.`id_perfil`,`usuarios`.`funcionario` FROM usuarios where `usuarios`.`id_usuario`=$us",$conexion);
$f=mysql_num_rows($sqlus);
if($f==1){
$row = @mysql_fetch_array($sqlus);
$id = $row['id_usuario'];
$log = $row['login'];
$pas = $row['pass'];
$per = $row['id_perfil'];
...
$sqlper=mysql_query("SELECT `id_perfil`, `nom_perfil` FROM `perfil` where `id_perfil` = $per",$conexion);
$p=mysql_num_rows($sqlper);
if($p==1){
$rows = @mysql_fetch_array($sqlper);
$nom = $rows['nom_perfil'];
}
...
$rs=$base->dosql($sql);
		echo"<center><p class=\"Estilo12\">EDITAR USUARIO:</p></center><BR>";
   echo"<center><form id=\"form1\" name=\"form1\" method=\"post\" action=\"../logica/updateusuario.php\" onsubmit=\"return validar()\">
   <input type=\"hidden\" name=\"id\" value=\"$id\" />
  <table width=\"420\" border=\"1\">
    <tr>
      <td width=\"155\"><center><BR>LOGIN:<BR><BR></center> </td>
      <td width=\"232\"><label>
        <input type=\"text\" name=\"login\" value=\"$log\" />
      </label></td>
    </tr>
    <tr>
      <td><center><BR>PASSWORD:<BR><BR></center> </td>
      <td><label>
        <input type=\"password\" name=\"pass\" value=\"$pas\" />
      </label></td>
    </tr>
	<tr>
      <td><center><BR>CONFIRMAR PASSWORD:<BR><BR></center> </td>
      <td><label>
        <input type=\"password\" name=\"cpass\" value=\"$pas\" />
      </label></td>
    </tr>
    <tr>
      <td><center><BR>PERFIL:<BR><BR></center> </td>
      <td>
          <select name=\"perfil\">";
            while(!$rs->EOF)
{
		...
		echo"<option value=\"$iperfil\"selected=\"$nom\">$perfil</option>";
		$rs->MoveNext();
}		
echo"</tr>
    <tr>
      <td><center><BR>FUNCIONARIO:<BR><BR></center> </td>
      <td><label>
        <input type=\"text\" name=\"funcionario\" value=\"$fun\"/>
      </label></td>
    </tr>
	<tr>
