...
<title>Ver Permisos de Perfil</title>
...
$pp=$_POST['permisoperfil'];
echo"<center><p class=\"Estilo12\">EDITAR PERMISO Y PERFIL: </p></center><BR>";
$base = NewADOConnection('mysql');
			$base->Connect('localhost','root','','plico');
			$db = mysql_connect('localhost','root','','plico');
			mysql_select_db('plico',$db); 
...
LEFT JOIN `plico`.`permisos` ON `permisos_perfil`.`id_permiso` = `permisos`.`id_permiso` where `permisos_perfil`.`id_permisoperfil`=$pp";
$query=mysql_query($sql,$db);
$filas=@mysql_num_rows($query);
if($filas==1){
$pager = new ADODB_Pager($base,$sql);
        echo"<center>";
        $pager->Render($rows_per_page=1);
        echo"</center>";
		echo"<center>OPCIONES:</center>";
		echo"<center>";
		echo'<table>';
		echo"<td><center><a href=\"../logica/borrapp.php?ID=$pp\"><IMG  src=\"../presentacion/borrar.jpg\" onclick=\"return confirmar('¿Está seguro que desea eliminar el registro?')\"></a></center></td>\n";	
			echo'</table>';
			echo"</center><BR><BR>";
			echo"<center><a href=\"../logica/edperfper.php\"><IMG  src=\"../presentacion/volver.jpg\" ></a></center><BR><BR>";
}
else{
echo"<center><img src=\"../presentacion/Advertencia.gif\"/></center>";
echo"<center>DATOS ERRONEOS DEBE INGRASAR UN NUMERO VALIDO</center><BR>";
echo"<center><a href=\"../logica/edperfper.php\"><IMG  src=\"../presentacion/volver.jpg\" ></a></center>";
}		
echo"</div>
<div class=\"clear\">
</div>
