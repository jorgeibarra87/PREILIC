...
<title>Ver Perfiles y Premisos</title>
...
$consul=mysql_query("SELECT `permisos_perfil`.`id_perfil`,`permisos_perfil`.`id_permiso` FROM `permisos_perfil`where `permisos_perfil`.`id_perfil`='$perfil'and `permisos_perfil`.`id_permiso`='3'",$conexion);
$fil=mysql_num_rows($consul);
if($fila==1||$fil==1){
$base=new base('localhost','root','','plico');
$sql="SELECT `perfil`.`id_perfil`,`perfil`.`nom_perfil` FROM perfil";
$rs=$base->dosql($sql);
echo"<center><p class=\"Estilo12\">VER PERFILES Y PERMISOS: </p></center><BR><BR>";
echo"<center><p class=\"Estilo12\">PERFILES: </p></center><BR>";
echo"<center>ELIJA EL PERFIL PARA VER LOS PERMISOS:</center><BR>";
echo"<center><form id=\"form1\" name=\"form1\" method=\"post\" action=\"../logica/verpp.php\">";
echo"<center>";
...
echo"</center>";
echo"<center><BR><input type=\"submit\" name=\"Submit\" value=\" VER \" /></center><BR><BR>";
